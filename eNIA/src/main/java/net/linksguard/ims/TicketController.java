...
import net.linksguard.ims.dao.StatusDao;
...
*/
    ...
    public String list(@PathVariable(required = false) Optional<Integer> id,
        ...
        if (id.isPresent()) {
            ...
            if (id.get() == 1) {
                model.addAttribute("tickets", imsDao.findAllByAssignedPersonId(customUserDetails.getId().intValue()));
    ...
    public String viewTicket(@AuthenticationPrincipal CustomUserDetails customUserDetails, Model model, @PathVariable String id) {
        Optional<ImsTicket> tickets = imsDao.findById(Integer.valueOf(id));
    ...
    public String updateTicket(@ModelAttribute  ImsTicket newTicket, Errors errors,
    ...
    public String displayCreateTicket(Model model) {
        ...
        model.addAttribute("ticket", new ImsTicket());
        model.addAttribute("groups", groupDao.findAll());
        model.addAttribute("severities", severityDao.findAll());
        model.addAttribute("statuses", statusDao.findAll());
        model.addAttribute("subcategory", categoryDao.findCategoryByCategoryTypeEquals("Sub"));
    ...
    public String processCreateTicket(@ModelAttribute  ImsTicket newTicket, Errors errors,
        ...
        ImsTicket insertedTicket = imsDao.save(newTicket);
